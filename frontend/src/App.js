...
function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      axios.get(API_URL + '/laporan/ringkasan').catch(() => ({data:{pelanggan:0,invois:0,jumlahHasil:0,belumBayar:0}})),
...
function PelangganPage() {
  ...
  const fetchData = async () => { const r = await axios.get(API_URL+'/pelanggan').catch(()=>({data:[]})); setList(Array.isArray(r.data)?r.data:[]); };
  useEffect(() => { fetchData(); }, []);
  const save = async (e) => {
    e.preventDefault();
    try { await axios.post(API_URL+'/pelanggan', form); setMsg('Berjaya!'); setShowForm(false); setForm({nama:'',telefon:'',emel:'',alamat:''}); fetchData(); setTimeout(()=>setMsg(''),3000); }
...
function PenggunaPage() {
  ...
  const [msg, setMsg] = useState('');
  const fetchData = async () => { const r = await axios.get(API_URL+'/pengguna').catch(()=>({data:[]})); setList(Array.isArray(r.data)?r.data:[]); };
  useEffect(()=>{ fetchData(); },[]);
  const save = async (e) => {
    e.preventDefault();
    try { await axios.post(API_URL+'/pengguna',form); setMsg('Berjaya!'); setShowForm(false); setForm({username:'',nama:'',password:'',role:'pengguna'}); fetchData(); setTimeout(()=>setMsg(''),3000); }
    catch(err) { setMsg('Ralat: '+(err.response?.data?.message||'Gagal')); }
  };
  return (
    <div className='page'>
